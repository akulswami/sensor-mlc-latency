...
int main(int argc, char **argv) {
    ...
    while (!stop_flag) {
        ...
        /* BINARY-FAST PATH: do NOT read MLC0_SRC. The 2-class config
         * guarantees every INT1 rising edge is a binary state transition,
         * so we toggle the decision GPIO unconditionally. mlc_src is set
         * to a sentinel for the host_dt log only (no I2C read). */
        uint8_t mlc_src = 0xFF;  /* sentinel: "not read" */

        ++event_count;

        bool current_motion_state = !last_motion_state;  /* flip every INT */

        if (1) {  /* always a transition in binary-fast mode */
