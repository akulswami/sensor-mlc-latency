...
The sensor is an STMicroelectronics LSM6DSOX 6-axis IMU on an Adafruit breakout (PID 4438; onboard 3.3 V regulation and 10K I²C pull-ups), connected via I²C bus 7 (pins 3 and 5 on the 40-pin header, address 0x6A). All three pipelines configure the accelerometer identically: `CTRL1_XL = 0x50` (ODR_XL = 208 Hz, FS_XL = ±2 g, LPF2 disabled). The Machine Learning Core (MLC) is enabled at 104 Hz (the configured accelerometer/MLC output data rate; distinct from the gyroscope_odr field, which reports 26 Hz in the ST MEMS Studio export) with a custom 2-class motion/still classifier trained in STMicroelectronics MEMS Studio (75-sample windows; MLC config header `mlc_motion_w75.h`, auto-generated from the MEMS Studio JSON export). MLC output is routed to INT1.
