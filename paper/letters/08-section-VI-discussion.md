...
Two secondary findings carry safety-critical weight. First, the MLC pipelines are multimodal at idle (§V.B): the mlc/idle p95 of 1,781 µs is 2.6× its median, a factor that vanishes under a unimodal-Gaussian assumption. For a "worst latency observed with probability 1 − ε" specification, the upper mode — not the median — is the relevant quantity. We report the structure as an observation; the mechanism requires kernel-level (ftrace) instrumentation beyond this study's scope.
...
Results are specific to a single platform (Jetson Orin Nano), sensor IC family (ST LSM6DSOX), bus protocol (I²C at 400 kHz, not SPI), accelerometer ODR (208 Hz), and MLC configuration (2-class, 75-sample windows). The structural findings — I²C-read dominance, multimodal distributions, the 706.5 ms cadence — should transfer to similar ARM-edge + ST-MEMS combinations but require empirical confirmation; SPI access in particular would reduce per-transaction overhead and could invert the ordering. A pre-registered chrt+taskset scheduling ablation (v7.5) was specified but not activated for the confirmatory campaign; pilot-scale data suggested RT scheduling could roughly halve MLC contention latency, making this the most concrete next step.
