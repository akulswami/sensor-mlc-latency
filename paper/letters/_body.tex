...
\begin{figure}[!t]
  \centering
  \includegraphics[width=0.62\columnwidth]{figure_2_setup_full.jpeg}
...
First, the MLC pipelines are multimodal at idle (Section~V.B): the mlc/idle p95 of 1,780 $\mu$s is 2.6$\times$ its median, a factor that vanishes under a unimodal-Gaussian assumption. For a ``worst latency observed with probability 1 $-$ $\varepsilon$'' specification, the upper mode, not the median, is the relevant quantity.
