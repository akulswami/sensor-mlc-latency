...
The sensor is an STMicroelectronics LSM6DSOX 6-axis IMU on an Adafruit breakout (PID 4438; onboard 3.3 V regulation and 10K I\textsuperscript{2}C pull-ups) on I\textsuperscript{2}C bus 7 (pins 3/5, address 0x6A) at 400 kHz (Fast Mode, Jetson default). All three pipelines configure the accelerometer identically (CTRL1\_XL = 0x50: 208 Hz, $\pm$2 g, LPF2 off). The MLC runs at 104 Hz (EMB\_FUNC\_ODR\_CFG\_C = 0x35: MLC\_ODR = 0b11, half the 208 Hz accelerometer ODR) with a custom 2-class motion/still classifier (75-sample windows, config mlc\_motion\_w75.h) trained in ST MEMS Studio, with output routed to INT1.
