...
**Status:** Pre-registered. Zenodo DOI: 10.5281/zenodo.20401819 (https://doi.org/10.5281/zenodo.20401819).
...
The full pre-registered confirmatory campaign (81 blocks, `confirmatory-2026-05-26`, captured 2026-05-26 13:25-20:25 PDT, committed in `becdc3c`) has been executed and statistically analyzed. The H1', H2', H3', H5' analyses were committed in `becdc3c`; the H6' analysis in `801991d`. This amendment formalizes the H7' analysis result, committed in `7ff5e40` (2026-05-27 morning PDT).

All five other testable confirmatory hypotheses (H1', H2', H3', H5', H6') are SUPPORTED at family-wise α=0.05 (Holm-Bonferroni corrected across {H1', H2', H3', H6'} for p-value tests; H5' is TOST equivalence, reported separately). v7.10 records the H7' result against the same campaign data; no new data was collected for this amendment.

---

### Reason for this amendment

H7' was introduced in v7.6 Change 6 (Zenodo DOI 10.5281/zenodo.20400025) as a directional secondary-outcome hypothesis:
...
> H7'₁: stability(MLC | i2c-contention) < stability(MLC | idle)
>
> Test: one-sided proportion test (chi-square or Fisher's exact at the per-trial level), Holm-Bonferroni corrected across the full set of confirmatory hypotheses.

The empirical result from the confirmatory campaign falsifies the pre-registered direction.

---

### Empirical finding

Per `data/processed/confirmatory-2026-05-26/h7_stability_results.json` (committed in `7ff5e40`), generated by `code/analysis/run_h7_stability.py`:

**Per-condition tally (MLC pipeline only, 9 blocks × 60 trials = 540 trials per cell):**

| Condition | Stable | Unstable (no_d1 + multi_d1) | Total | Stability rate |
|---|---|---|---|---|
| mlc / idle | 525 | 15 (2 + 13) | 540 | **97.22%** |
| mlc / i2c-contention | 534 | 6 (1 + 5) | 540 | **98.89%** |
| mlc / stress (reference, not in H7' test) | 527 | 13 (1 + 12) | 540 | 97.59% |

**H7' test result (Fisher's exact, MLC pipeline, idle vs i2c-contention):**

| Quantity | Value |
|---|---|
| Δ (contention − idle) | +1.67 percentage points (OPPOSITE to pre-reg prediction) |
| Odds ratio (idle vs contention) | 0.3933 |
| Fisher 1-sided, alternative='greater' (pre-reg direction) | p = 0.9874 |
| Fisher 2-sided (reference) | p = 0.0755 |
| Chi-square 2-sided (reference) | χ² = 3.108, p = 0.0779 |

The one-sided test in the pre-registered direction cannot reject H7'₀ (p = 0.9874, far above α=0.05). The two-sided test is marginal (p = 0.0755) but, critically, the direction of the observed effect is opposite to the pre-registered prediction. Both stressed conditions (i2c-contention 98.89%, cpu-stress 97.59%) show slightly higher stability than idle (97.22%), suggesting idle may be the outlier rather than contention.

**Holm-Bonferroni across the full pre-registered family {H1', H2', H3', H6', H7'} at α=0.05:**

| Rank | Hypothesis | Raw p | Holm-adjusted p | Threshold | Rejected? |
|---|---|---|---|---|---|
| 1 | H6' | 0 (below float64 epsilon) | 0 | 0.0100 | YES |
...
H5' (TOST equivalence) is reported separately: median diff +23.3 µs, 90% CI [22.7, 23.7] ⊂ [−30, +30], EQUIVALENT.

---

### Change 1: §2 H7' is formally falsified

v7.6 Change 6 introduced H7' as the directional hypothesis stated above. This hypothesis is falsified by the confirmatory campaign result. H7' is replaced by:

> **H7' (NULL, post-falsification 2026-05-27):** The per-trial classifier-stability rate of the MLC bank-switch pipeline at idle and under I²C bus contention is statistically indistinguishable in the pre-registered direction. The observed direction (contention slightly more stable than idle by 1.67 pp) is opposite to the pre-registered prediction; the two-sided test is marginal (p=0.076, n.s. at α=0.05).
>
> H7'₀ (null, retained as the empirical finding): one-sided Fisher's exact in the pre-reg direction fails to reject; two-sided test does not reach α=0.05.
> H7'₁ (alternative, falsified): stability(MLC | i2c-contention) < stability(MLC | idle).

The empirical finding (no statistically significant degradation in the predicted direction) is reported as such. **No re-test of the falsified H7'₁ is planned.**
...
- §1 research question
- §2 hypotheses H1', H2', H3' (all SUPPORTED per confirmatory campaign)
- H4' status (retired in v7.6 as falsified; this amendment does not revisit)
- H5' (SUPPORTED per confirmatory campaign; TOST equivalence)
- H6' (SUPPORTED per confirmatory campaign)
- §3 design, §4 task, §5 pipelines, §6 outcomes, §7 randomization, §8 stress conditions
- §9 accuracy parity gate
- §11 exclusion criteria
- §12 statistical machinery (no new test introduced; H7' used pre-registered Fisher's exact)
- nvpmodel mode 3 / jc_eff threshold (v7.6)
- Block-order seed (v7.8)
- extract_latency_v7.py mlc-binary support (v7.9)

This amendment is purely a record of an empirical falsification. No code changes, no methodology changes, no data re-collection. The confirmatory campaign data is unchanged; the analysis script (`run_h7_stability.py`) is committed and reproducible.

---

### Mechanism observation (post-hoc, exploratory, NOT pre-registered)

The 1.67 pp gap is in the marginally-significant range (two-sided p=0.076). The observed direction — both stressed conditions slightly more stable than idle — admits at least two non-exclusive hypotheses for future investigation, neither pre-registered here:

1. **Steady-state load reduces timing-edge artifacts.** Under idle, the system has more headroom for occasional CPU spikes (background activity, daemons) that may cause the MLC pipeline to read MLC0_SRC at unexpected times relative to the silicon's decision window. Under contention or stress, the system is in a more consistent state of load, which may smooth out these timing-edge events.

2. **Sampling noise on small counts.** Unstable counts are small (15, 6, 13 across the three conditions). The two-sided p=0.076 is consistent with a null effect, and the direction could be sampling variation.

Neither hypothesis is pre-registered in this amendment; both are noted only as candidate mechanisms for the paper's exploratory discussion section. The paper-grade claim is the falsification itself: the pre-registered direction is not supported, and the observed direction is opposite.

---

### Falsification record (cumulative, as of v7.10)

Three pre-registered hypotheses have been formally falsified during this study, each with empirical evidence:

| Hypothesis | Falsified in | Direction observed | Pre-reg DOI of falsification record |
|---|---|---|---|
| H1 (median MLC < median host, no stress) | v7.5 (btest scale, 2026-05-25) | Reversed: median(host) < median(MLC) | 10.5281/zenodo.20389914 |
| H4' (mlc idle energy < host idle energy) | v7.6 (long-duration jc-effective, 2026-05-26) | Null: gap is within ±50 mW noise floor | 10.5281/zenodo.20400025 |
| **H7' (MLC stability under contention < idle)** | **v7.10 (confirmatory, 2026-05-27)** | **Reversed: contention slightly more stable than idle (+1.67 pp, two-sided p=0.076)** | **[this amendment's DOI]** |

Additionally, one pre-registered prediction (not a hypothesis) was falsified:

- **v7.5 Change 1** predicted "smaller energy delta under nvpmodel mode 3 because baseline near max CPU frequency." Falsified by H6' result (+3420 mW under mode 3, comparable to btest-scale +3000 mW). Recorded in `data/processed/confirmatory-2026-05-26/h6_energy_results.json` (commit `801991d`).

Each falsification is preserved with empirical evidence and an external Zenodo timestamp. The pre-registration discipline keeps the audit trail intact regardless of which hypotheses survived and which did not.

---

### Substantive implication for the paper

The H7' falsification refines the safety-critical interpretation of the I²C contention findings. H2' and H3' establish that I²C contention adds substantial latency to both pipelines (host +253 µs HL shift, MLC +753 µs HL shift, MLC degrades 391 µs MORE than host). H7' establishes that this latency penalty does NOT come with a classifier-stability penalty — if anything, contention slightly improves stability.

For systems specifying latency budgets for on-sensor inference (e.g., the safety-critical exoskeleton control loops in Razmi & Shojaei 2026, arXiv:2602.21418): I²C bus contention is a latency hazard, not a reliability hazard. The host-side classifier under contention is slower, not less reliable. This is a stronger and more nuanced specification claim than "stress degrades the MLC pipeline in every way."

The paper's §VI discussion should note this finding alongside the H5'+H6' control pair. The discriminative-control design that pre-registration v7.5/v7.6 set up now reports cleanly:

- **CPU stress:** detectable on energy (H6'), null on latency (H5'), null on classifier stability (H7' for the contention case, observed direction null/positive)
- **I²C contention:** significant on latency (H2', H3'), null/positive on classifier stability (H7')
- **Conclusion:** "stress" is not a single thing; the safety-critical question must be specific about which bus or resource is contended.
